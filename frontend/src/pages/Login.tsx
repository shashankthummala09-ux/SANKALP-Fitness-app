...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.error || t('auth.loginFailed', 'Login failed. Please verify your credentials.');
      setError(errMsg);
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="w-full max-w-md space-y-8 glass-panel p-8 sm:p-10 border border-white/10 shadow-2xl relative z-10">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-brand-muted">
            {t('auth.loginSubtitle', 'Log in to track workouts and discover local gyms')}
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center">
              <input
                ...
                name="remember-me"
                type="checkbox"
                className="h-4 w-4 rounded border-white/10 bg-brand-dark/50 text-brand-primary focus:ring-brand-primary focus:ring-offset-brand-dark cursor-pointer"
              />
              <label htmlFor="remember-me" className="ml-2 block text-brand-muted cursor-pointer">
          ...
          <div>
            ...
            >
              {submitting ? t('auth.loggingIn', 'Signing In...') : t('auth.loginButton', 'Sign In')}
            </button>
          </div>
        </form>
        ...
        <div className="text-center text-sm text-brand-muted mt-6">
          ...
          <Link to="/register" className="font-semibold text-brand-primary hover:text-brand-secondary transition-colors duration-200">
